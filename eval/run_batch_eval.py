# eval/run_batch_eval.py
# Examples:
#   python run_batch_eval.py --course networking --limit 10 --temperature 0.2 --top_k 4
#   python run_batch_eval.py --course architecture --limit 10 --temperature 0.2 --top_k 4
#   python run_batch_eval.py --course machine_learning --temperature 0.7 --top_k 20
...
COURSE_TO_CSV = {
    "networking": str(REPO_ROOT / "eval/networking_eval.csv"),
    "architecture": str(REPO_ROOT / "eval/architecture_eval.csv"),
    "machine_learning": str(REPO_ROOT / "eval/machine_learning_eval.csv"),
}
...
def ask_gpt(system: str, q: str, ctx: str) -> Tuple[str, float]:
    ...
    llm = get_gpt_llm(temperature=current_args.temperature)
    ...
    for m in claude_candidates():
        try:
            ...
            resp = client.messages.create(
                model=m, max_tokens=700, temperature=current_args.temperature,
...
def parse_args():
    ...
    parser.add_argument(
        "--course",
        choices=["networking", "architecture", "machine_learning"],
        default=DEFAULT_COURSE,
        help="Course to evaluate: networking, architecture, or machine_learning."
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.2,
        help="Sampling temperature to use for model generation."
    )
    parser.add_argument(
        "--top_k",
        type=int,
